...
BEGIN
    ...
    (@cSpanish, 'Greetings & Introductions', 'Learn essential Spanish greetings:
- ¡Hola! (Hello!)
...
- Mucho gusto (Nice to meet you)', 1),
    (@cSpanish, 'Numbers 1-100 & Counting', 'Master counting in Spanish:
...
- ESTAR is used for temporary states and locations: "Yo estoy en Madrid."', 3),
    (@cSpanish, 'Ordering at a Restaurant', 'Navigate a Spanish restaurant:
...
BEGIN
    INSERT INTO Lessons (CourseID, Title, Content, SortOrder) VALUES
    (@cFrench, 'Les Salutations & Politesse', 'Learn polite French expressions:
...
- Merci beaucoup (Thank you very much)', 1),
    (@cFrench, 'Le Passé Composé', 'Master past actions in French:
...
- Je suis allé à Paris (I went to Paris with être for movement verbs).', 2),
    (@cFrench, 'Au Café & À la Boulangerie', 'Ordering food and drinks in Paris:
...
- C''est combien ? (How much is it?)', 3),
    (@cFrench, 'Se Déplacer dans la Ville', 'Navigating the city and public transport:
...
BEGIN
    INSERT INTO Lessons (CourseID, Title, Content, SortOrder) VALUES
    (@cGerman, 'Begrüßung & Kennenlernen', 'German basics for first conversations:
...
Tip: Always learn the noun together with its article!', 2),
    (@cGerman, 'Im Restaurant & Bestellen', 'Ordering food in Germany:
...
BEGIN
    INSERT INTO Lessons (CourseID, Title, Content, SortOrder) VALUES
    (@cJapanese, 'Hiragana Basics: あ い う え お', 'Master the foundational Japanese phonetic alphabet:
- あ (a), い (i), う (u), え (e), お (o)
- か (ka), き (ki), く (ku), け (ke), こ (ko)
Practice stroke order from top-to-bottom, left-to-right.', 1),
    (@cJapanese, 'Katakana & Loanwords: ア イ ウ エ オ', 'Katakana is used for foreign words and emphasis:
- コーヒー (Koohii - Coffee)
- アメリカ (Amerika - America)
- レストラン (Resutoran - Restaurant)', 2),
    (@cJapanese, 'Self-Introduction: 自己紹介 (Jikoshoukai)', 'Introduce yourself in Japanese:
...
BEGIN
    INSERT INTO Lessons (CourseID, Title, Content, SortOrder) VALUES
    (@cChinese, 'The Four Tones & Pinyin', 'Master the 4 fundamental tones in Mandarin:
- 1st Tone (High flat): mā (Mother)
- 2nd Tone (Rising): má (Hemp)
- 3rd Tone (Falling-rising): mǎ (Horse)
- 4th Tone (Sharp falling): mà (Scold)', 1),
    (@cChinese, 'Essential Greetings: 你好 (Nǐ hǎo)', 'Everyday conversational Chinese:
- 你好 (Nǐ hǎo - Hello)
- 谢谢 (Xièxie - Thank you)
- 不客气 (Bú kèqì - You are welcome)
- 再见 (Zàijiàn - Goodbye)', 2),
    (@cChinese, 'Numbers & Shopping: 这个多少钱？', 'Numbers and asking for prices:
...
BEGIN
    INSERT INTO Lessons (CourseID, Title, Content, SortOrder) VALUES
    (@cKorean, 'Hangul Masterclass: ㄱ ㄴ ㄷ & ㅏ ㅓ ㅗ', 'Learn the scientific Korean alphabet:
...
Combine consonants and vowels into blocks: 한 (h-a-n) 글 (g-eu-l).', 1),
    (@cKorean, 'Essential K-Drama Phrases', 'Popular expressions used in daily life and dramas:
- 안녕하세요 (Annyeonghaseyo - Hello)
- 감사합니다 (Gamsahamnida - Thank you)
- 대박! (Daebak! - Awesome / Jackpot!)
- 화이팅! (Hwaiting! - Fighting / You got this!)', 2),
    (@cKorean, 'Ordering Korean BBQ & Food', 'Food vocabulary:
...
- Per favore (Please) & Grazie mille (Thank you so much)', 1),
    (@cItalian, 'Al Ristorante & Caffè', 'Ordering food like a local:
...
- Por favor (Please) & Obrigado / Obrigada (Thank you)', 1),
    (@cPortuguese, 'Na Praia & Na Cidade', 'Everyday conversation in Rio & São Paulo:
...
BEGIN
    INSERT INTO Lessons (CourseID, Title, Content, SortOrder) VALUES
    (@cArabic, 'The Arabic Alphabet: أ ب ت ث', 'Arabic is written right-to-left:
- Alif (أ), Baa (ب), Taa (ت), Thaa (ث)
Letters connect in cursive depending on position (initial, medial, final).', 1),
    (@cArabic, 'Essential Greetings & Courtesy', 'Polite phrases:
